...
from file_handler import get_subtitle_filename_from_clip, check_copy_conditions, \
    ...
    for channel in TV_CHANNELS_STATE:
        for clip in channel.clips:
            for lang in channel.subtitle_languages:
                cur_file_to_check = get_subtitle_filename_from_clip(clip, lang)
                if cur_file_to_check not in channel.subtitles_on_m_server \
                    ...
                    copy2(os.path.join(channel.channel_sub_src_dir, lang, cur_file_to_check),
                             os.path.join(channel.channel_m_sub_dir, cur_file_to_check))
...
def copy_updated_subtitles():
        for clip in channel.clips:
            for lang in channel.subtitle_languages:
                cur_file_to_check = get_subtitle_filename_from_clip(clip, lang)
                if cur_file_to_check in channel.subtitles_on_m_server \
                        and cur_file_to_check in channel.subtitles_on_source:
                    size_cond, mod_date_cond = check_copy_conditions(channel.channel_sub_src_dir, channel.channel_m_sub_dir, cur_file_to_check, lang)
...
def delete_unused_subtitles():
    for channel in TV_CHANNELS_STATE:
        for subtitle in channel.subtitles_on_m_server:
            if get_clip_filename_from_subtitlename(subtitle) not in [c.clip_name for c in channel.clips]:
                os.remove(get_subtitle_server_full_path(channel.channel_m_sub_dir, subtitle))
                create_log_record(prepare_log_message(channel.channel_name, subtitle, SUBTITLE_DELETION_EVENT_NAME), logging.INFO)

TASKS.append(update_data)
"""
